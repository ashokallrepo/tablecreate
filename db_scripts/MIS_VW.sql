...
FROM MIS M 
INNER JOIN BROKER B ON B.ID=M.BROKER -- (Not Mandatory)
INNER JOIN DEPARTMENT DEP ON DEP.ID=M.DEPT_NAME -- (Mandatory)
INNER JOIN SUB_DEPT SDEP ON SDEP.DEPARTMENT = DEP.ID AND SDEP.ID=M.SUB_DEPT --(Not Mandatory)
INNER JOIN FIELD_STAFF FS ON FS.ID=M.FIELD_STAFF -- (Mandatory)
INNER JOIN INSURED INRD ON INRD.ID= M.INSURED -- (Mandatory)
INNER JOIN INSURER INER ON INER.ID=M.INSURER -- (Mandatory)
INNER JOIN ESTIMATED_CLAIM_AMT ECA ON ECA.ID=M.EST_CLAIM_AMT -- (Mandatory)
INNER JOIN REGISTRATION_BRANCH RB ON RB.ID = M.REG_BRANCH -- (Mandatory)
INNER JOIN SURVEYOR SUR  ON SUR.ID = M.SURVEYOR -- (Mandatory)
INNER JOIN SOURCE_OF_INSTRUCTION SOI  ON SOI.ID=M.SRC_OF_INST; -- (Mandatory)
